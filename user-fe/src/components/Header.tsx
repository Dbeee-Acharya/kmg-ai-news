...
export default function Header({ bgColor = 'bg-white' }: HeaderProps) {
  ...
  const activeBg = bgColor === 'transparent' 
    ...
    : bgColor + ' backdrop-blur-md bg-white/80 shadow-sm border-zinc-100';
      ...
      <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-3 group">
          <span className="text-xl font-black text-zinc-900 tracking-tight">
            eKantipur <span className="text-zinc-500 font-medium">Fact Check</span>
          </span>
        </Link>
        
        <nav className="hidden md:flex items-center gap-8">
          <Link to="/" className="text-sm font-bold text-zinc-900 hover:text-blue-600 transition-colors uppercase tracking-widest">
            Timeline
          </Link>
          <a 
